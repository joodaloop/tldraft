...
export interface TypographyOptions {
  /**
   * Bidirectional arrow character.
   * @default '—'
   */
  biArrow: false | string;

  /**
   * Bidirectional arrow character.
   * @default '—'
   */
  thickBiArrow: false | string;
...
export const thickBiArrow = (override?: string) =>
  textInputRule({
    find: /<=>$/,
    replace: override ?? "⇔",
  });
...
export const Typography = Extension.create<TypographyOptions>({
  ...
  addInputRules() {
    ...
    if (this.options.biArrow !== false) {
      rules.push(biArrow(this.options.biArrow));
    }
    ...
    if (this.options.thickBiArrow !== false) {
      rules.push(thickBiArrow(this.options.thickBiArrow));
    }
